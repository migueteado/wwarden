...
export const seedCategories = async (prismaClient: PrismaClient) => {
  ...
  for (const category of categories) {
    promises.push(prismaClient.categories.create({ data: category }));
  }

  const seededCategories = await Promise.all(promises);

  console.log(`Categories Added: ${seededCategories.length}`);
